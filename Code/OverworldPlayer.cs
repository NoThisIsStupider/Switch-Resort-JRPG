...
public class OverworldPlayer : KinematicBody2D
{
    ...
    public async void MoveToNewMap(Warp entrance, string targetMap, int exitNumber)
    {
        ...
        GetNode<BetweenMaps>("/root/BetweenMaps").PrepareForMapChange(exitNumber, facingDir, entrance.outDirectionNormalized);
        GetNode<ScreenEffects>("/root/ScreenEffects").BeginEffect(EffectTypes.FadeOut, TRANSITION_FADE_LENGTH);
        await ToSignal(GetTree().CreateTimer(TRANSITION_FADE_LENGTH), "timeout");

        GetTree().ChangeScene($"res://Scenes/Maps/{targetMap}");
    }
    ...
    public async void OnMapEnter(Warp exit, Vector2 playerMoveDir, Vector2 entranceInDirection)
    {
        ...
        GetNode<ScreenEffects>("/root/ScreenEffects").BeginEffect(EffectTypes.FadeIn, TRANSITION_FADE_LENGTH);
